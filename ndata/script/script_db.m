...
filename_country = 'tr_country.csv';
filename_out = 'outflows_';
...
header = {'country_dest', 'country_origin', 'total', 'year'};
%header = {'country_dest', 'country_origin', 'total'};
...
%Lê o .csv de países
[COD_COUNTRY,NAME_COUNTRY,COD_REGION] = importfile(filename_country,2, 233);
new_data = [];
%Filtragem dos dados
    ...
    for cd=1:length(data_country_dest)
        for co=1:length(data_country_origin)
            ...
            if ~cellfun(@isnan,total)
                ...
                %encontra o índice do país
                id_cdest = find(COD_COUNTRY == cell2mat(data_country_dest(cd)));
                id_corigin = find(COD_COUNTRY == cell2mat(data_country_origin(co)));
                
                %obtém o nome
                cname_dest = NAME_COUNTRY(id_cdest);
                cname_origin = NAME_COUNTRY(id_corigin);

                new_data = [new_data; [cname_dest cname_origin total data_year(i)]];
...
cell2csv(file_out, new_data, true);
